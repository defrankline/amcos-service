...
public final class GlobalMethod {
    ...
    private final String uploadDirectory = Config.UPLOADS_DIRECTORY;
    ...
    public static LocalDate stringToDate(String dateToConvert) {
        String dateString = dateToConvert != null ? dateToConvert : LocalDate.now().toString();
        DateTimeFormatter format = DateTimeFormatter.ofPattern("yyyy-MM-dd");
        LocalDate date = LocalDate.now();
        try {
            date = LocalDate.parse(dateString, format);
        } catch (Exception e) {
            System.out.println("Dirty Date");
        }
        return date;
    }
    ...
    public static String convertFileToBase64(File file) {
        ...
        try {
            if (file.exists()) {
                ...
                String uploadDirectory = Config.UPLOADS_DIRECTORY;
